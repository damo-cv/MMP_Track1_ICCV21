...
def save(res, output, dist_th, ios_th):
    results = np.load(res)
    loaded_trk_ids = np.unique(results[:, 1])
    results = track_nms(results, dist_th, ios_th)
...
def main():
    ...
    parser.add_argument('--dist_th',
                        type=float, default=30)
    parser.add_argument('--ios_th', type=float, default=0.5)
    ...
    for res in ress:
        ...
        tasks.append((res, output, args.dist_th, args.ios_th))
