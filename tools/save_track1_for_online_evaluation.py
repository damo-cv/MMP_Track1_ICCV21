...
def save(res_path, output):
    ...
    for res in ress:
        fid = int(res[0])
        fid2res[fid].append(res[1:5].tolist())
    scene_name = res_path.parent.name
    for fid in range(MAX_FID_DICT[scene_name] + 1):
        out_path = output / ('topdown_%05d.csv' % fid)
        if fid not in fid2res:
        ...
        for fr in fid2res[fid]:
            tid = int(fr[0])
            out.append([tid, fr[1], fr[2], fr[3]]) #xyxy
        out_df = pd.DataFrame(out)
        out_df.to_csv(out_path, header=None, index=False)
